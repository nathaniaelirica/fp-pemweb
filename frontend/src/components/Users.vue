<template>
    <div>
                    ...
                    <table class="w-full text-sm text-left text-gray-500 dark:text-gray-400">
                        ...
                        <tbody>
                            <tr v-for="dashboard in App.dashboards" :key="dashboard.id" class="bg-white border-b dark:bg-gray-800 dark:border-gray-700">
                                ...
                                <td>
                                    <span @click="showEditDashboardMenu(dashboard.id)" class="material-symbols-outlined mr-3 cursor-pointer">
                                        edit
                                    </span>
                                    <span @click="deleteDashboard(dashboard.id)" class="material-symbols-outlined text-red-700 cursor-pointer">
                                        delete
                                    </span>
                                </td>
                            </tr>
        ...
        <transition>
            <EditUser v-show="this.App.menu.edit_dashboard.show"/>
        </transition>
    </div>
</template>
...
export default {
    ...
    methods:{
        ...
        const res = await axios.get(`http://127.0.0.1:3001/dashboard/redirect/${link}`).catch((err)=>{
          console.log(err)
        })
        window.location.href= res.data.links.destination
      },
      async showEditDashboardMenu(dashboard_id) {
